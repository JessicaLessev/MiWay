...
foreach($inputArray as $instruction){
    $decodedInstruction = explode(" ", $instruction);
    $reg = $decodedInstruction[0];
    $registers = ensureElementInArray($reg, $registers);
    $op = $decodedInstruction[1];
    $opAmount = $decodedInstruction[2];
    $cond1 = $decodedInstruction[4];
    $condOp = $decodedInstruction[5];
    $cond2 = $decodedInstruction[6];
    
    if(!is_numeric($cond1)){
        $registers = ensureElementInArray($cond1,$registers);;
    }
    $cond = 'return '.$registers[$cond1].$condOp.$cond2.";";
    if(eval($cond)){
        $registers = handleOp($reg,$op,$opAmount, $registers);
    }
}

    echo max($registers);

function handleOp($reg,$op,$opAmount,$registers){
    $registers[$reg] = $op=="inc"? $registers[$reg] + (int)$opAmount:$registers[$reg] - (int)$opAmount;
    return $registers;
}

function ensureElementInArray($value,$registers){
    if(!array_key_exists($value,$registers)){
        $reg = $value;
        $registers[$reg] = 0;
    }
    return $registers;
